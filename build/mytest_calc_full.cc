...
TEST(CheckNumber, Err) {
  ...
  EXPECT_EQ(model1->check_number("10"), 0);
  EXPECT_EQ(model1->check_number("01"), -1);
  EXPECT_EQ(model1->check_number("01."), -1);
  EXPECT_EQ(model1->check_number("00."), -1);
  EXPECT_EQ(model1->check_number("0.001"), 0);
  EXPECT_EQ(model1->check_number("001"), -1);
  EXPECT_EQ(model1->check_number("100000.00001"), 0);
  EXPECT_EQ(model1->check_number("0.0000000001"), 0);
  EXPECT_EQ(model1->check_number("00"), -1);
  EXPECT_EQ(model1->check_number("00.."), -1);
  EXPECT_EQ(model1->check_number("0.0.1"), -1);
  EXPECT_EQ(model1->check_number("00.1"), -1);
  EXPECT_EQ(model1->check_number("0.1"), 0);
  EXPECT_EQ(model1->check_number("-0.1"), 0);
  EXPECT_EQ(model1->check_number("-3"), 0);
  EXPECT_EQ(model1->check_number("-0.0001"), 0);
  EXPECT_EQ(model1->check_number("-0.000000"), 0);
  EXPECT_EQ(model1->check_number("0.000000"), 0);
}
...
TEST(Calc, previous_next_ch) { //работает
  ...
  model1->prev_next_ch( str, 0, num);
  EXPECT_EQ(num[0] == '0', true);

  model1->prev_next_ch(str, 1, num);
  EXPECT_EQ(num[0] == '.', true);

  model1->prev_next_ch(str, 2, num);
  EXPECT_EQ(num[0] == '3', true);

  tmp = model1->prev_next_ch( str, -1, num);
  EXPECT_EQ(tmp == -1, true);

  tmp = model1->prev_next_ch(str, 5, num);
  EXPECT_EQ(tmp == -1, true);
}
...
// "sin(x)e+2"
// xe-x
  EXPECT_EQ(model1->s21_calc("xe+4", str_x, res),  0);
  EXPECT_EQ(res == 10000, true);
  EXPECT_EQ(model1->s21_calc("65e+x", str_x, res),  0);
