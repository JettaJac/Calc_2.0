...
//   model.SmartCalc_m("45mod4", x, res);
//   // test.SmartCalc_m("45mod4", x, res);
//   // model.SmartCalc_m("45mod4", x, res);
...
TEST(Calc, znak_1) {
  ...
  // MyTest *test;
  // test->SmartCalc_m->
  model1->SmartCalc_m("-(5+6)", x, num);
  // model->SmartCalc_m("-(5+6)", x, num);
  EXPECT_EQ(num,  -11);
}
...
TEST(Calc, matematika_simple) {
  ...
  model1->SmartCalc_m("5", x, res);  
  EXPECT_EQ(res == 5, true);

  str = "0.345+5";
  model1->SmartCalc_m(str, x, res);
  EXPECT_EQ(res == 5.345, true);

  model1->SmartCalc_m("5+7-3-2", x, res);
  EXPECT_EQ(res == 7, true);

  model1->SmartCalc_m("5+7-3*2", x, res);
  EXPECT_EQ(res == 6, true);

  model1->SmartCalc_m("5+7*3-3*2", x, res);
  EXPECT_EQ(res == 20, true);

  model1->SmartCalc_m("5+7-3/2", x, res);
  EXPECT_EQ(res == 10.5, true);

  model1->SmartCalc_m("3*(5+6)-2*4-(8*9/3-6)", x, res);
  EXPECT_EQ(res == 7, true);

  model1->SmartCalc_m("3*(5+6)-2*4-(8*9/3-6)/7", x, res);
  EXPECT_EQ(res - 22.4285714285714286 <= ERC, true);

  model1->SmartCalc_m("3*(5+6)-2*4-(8*9/3-6)/7-2*3", x, res);
  EXPECT_EQ(res - 16.4285714285714286 <= ERC, true);

  model1->SmartCalc_m("3-(5+6)*(2+4)-8*9-6/7-2*3", x, res);
  EXPECT_EQ(res -(-141.8571428571428571) <= ERC, true);

  tmp = model1->SmartCalc_m("(5+4-2*5)/(5-5)", x, res);
  EXPECT_EQ(tmp, -2);

  tmp = model1->SmartCalc_m("(4+10)/0*2+1", x, res);
  EXPECT_EQ(tmp, -2);

  model1->SmartCalc_m("80/2", x, res);
  EXPECT_EQ(res - 40 <= ERC, true);

  model1->SmartCalc_m("80/(2-10)", x, res);
  EXPECT_EQ(res - (-10) <= ERC, true);

  model1->SmartCalc_m("80/(-10)", x, res);
  EXPECT_EQ(res - (-8) <= ERC, true);

  model1->SmartCalc_m("-80/(-10)", x, res);
  EXPECT_EQ(res - 8 <= ERC, true);

  model1->SmartCalc_m("-(5+1)", x, res);
  EXPECT_EQ(res - (-6) <= ERC, true);

  model1->SmartCalc_m("-(-(5+2)*4)", x, res);
  ...
  model1->SmartCalc_m("sin(cos(log(5^2)))", x, res);
  
  EXPECT_EQ(res - 0.17115002489 <= ERC, true);

  model1->SmartCalc_m("sin(cos(tan(acos(asin(atan(log(1*ln(sqrt(5mod3^(-2)+x)))))))))", x, res);
  cout << res << endl;
  EXPECT_EQ(res - 0.742174 <= ERC, true);
  
  // 0.71188622115482
  model1->SmartCalc_m("sin(cos(tan(acos(1))))", x, res);
  cout << res << endl;
  EXPECT_EQ(res - 0.8414709848 <= ERC, true);

  model1->SmartCalc_m("sin(2)", x, res);
  cout << res << endl;
  EXPECT_EQ(res - 0.90929742682 <= ERC, true);

  model1->SmartCalc_m("sqrt((-2)*(-2))", x, res);
...
TEST(Calc, matematika_trig_1) {
  ...
  model1->SmartCalc_m("sin(5)", x, res);
  res = res - (-0.95892427466);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("cos(5)", x, res);
  res = res - (0.28366218546);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("cos(45)", x, res);
  res = res - (0.52532198881);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("cos(45)+sin(67)", x, res);
  ...
  model1->SmartCalc_m("1-cos(45)", x, res);
  ...
  model1->SmartCalc_m("acos(0.5)", x, res);
  res = res - (1.0471975511966);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("asin(0.5)", x, res);
  res = res - (0.523598775598299);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("atan(x)", x, res);
  res = res - (1.5490606199531);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("sin(x)", x, res);
...
TEST(Calc, matematika_log_1) {
  ...
  model1->SmartCalc_m("log(5)", x, res);
  res = res - (0.6989700043360189);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("ln(5)", x, res);
  res = res - (1.6094379124341004);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("log(45)", x, res);
  res = res - (1.6532125137753437);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("ln(45)+log(67)", x, res);
  res = res - (5.6327372924711462);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("(-log(45)+sin(67))-(ln(10)+log(56))", x, res);
  res = res - (-6.55950561275);
  EXPECT_EQ(res <= ERC, true);

  model1->SmartCalc_m("-ln(45-20)", x, res);
...
TEST(Calc, matematika_mod) {
  ...
  model1->SmartCalc_m("45mod4", x, res);
  EXPECT_EQ(res == 1, true);

  model1->SmartCalc_m("(2+3*5)mod(3*2)", x, res);
  EXPECT_EQ(res == 5, true);

  model1->SmartCalc_m("(2^2^2)mod3", x, res);
  EXPECT_EQ(res == 1, true);

  model1->SmartCalc_m("5mod2+33mod6*6", x, res);
  EXPECT_EQ(res == 19, true);

  model1->SmartCalc_m("(-7)mod(7mod4)", x, res);
  EXPECT_EQ(res == -1, true);
 }
...
TEST(Calc, matematika_pow) {
  ...
  model1->SmartCalc_m("2^(-1+1*3)", x, res);
  EXPECT_EQ(res == 4, true);

  model1->SmartCalc_m("2^(-1+1*3)+5*4", x, res);
  EXPECT_EQ(res == 24, true);

  model1->SmartCalc_m("2^(-2)", x, res);
  EXPECT_EQ(res == 0.25, true);

  model1->SmartCalc_m("(-3^(-3))", x, res);
...
TEST(Calc, matematika_x) {
  ...
  model1->SmartCalc_m("x^2+3*x", x, res);
  EXPECT_EQ(res == 40, true);

  model1->SmartCalc_m("3*x", x, res);
  EXPECT_EQ(res == 15, true);

  model1->SmartCalc_m("x*3", x, res);
  EXPECT_EQ(res == 15, true);

  model1->SmartCalc_m("3*x+2*x", x, res);
  EXPECT_EQ(res == 25, true);

  model1->SmartCalc_m("3*x+2+x", x, res);
  EXPECT_EQ(res == 22, true);

  model1->SmartCalc_m("-x", x, res);
  EXPECT_EQ(res == -5, true);

  char x2[3] = "-5";

  model1->SmartCalc_m("x^2+3*x", x2, res);
  EXPECT_EQ(res == 10, true);

  model1->SmartCalc_m("3*x", x2, res);
  EXPECT_EQ(res == -15, true);

  model1->SmartCalc_m("x*3", x2, res);
  EXPECT_EQ(res == -15, true);

  model1->SmartCalc_m("3*x+2*x", x2, res);
  EXPECT_EQ(res == -25, true);

  model1->SmartCalc_m("3*x+2+x", x2, res);
  EXPECT_EQ(res == -18, true);

  model1->SmartCalc_m("-x", x2, res);
  EXPECT_EQ(res == 5, true);
}
...
TEST(Calc, matematikaErr) {
  ...
  EXPECT_EQ(model1->SmartCalc_m("-cos(45)sin(67)", x, res) == -1, true);
  EXPECT_EQ(model1->SmartCalc_m("1 - cos(45)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("-cos(45)+sin(-67)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("-cos(+45)+sin(67)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("-ln(45)+sin(67)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("5mod4+sin(67)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("2^2^2", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("2^2^", x, res) == -1, true);
  EXPECT_EQ(model1->SmartCalc_m("2^(-1+1*3)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("2^(-1+1*3)+5*4", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("log(-45)", x, res) == -3, true);
  EXPECT_EQ(model1->SmartCalc_m("6mod(-4)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("(-5)mod(-4)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("(-5)mod(6mod4)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("(2+3*5)mod(3*2)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("atan(3*2)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("asin(5)", x, res) == -3, true);
  EXPECT_EQ(model1->SmartCalc_m("asin(0.4)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("acos(5)", x, res) == -3, true);
  EXPECT_EQ(model1->SmartCalc_m("acos(0.5)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("atan(5)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("sin(0.4)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("cos(5)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("tan(5)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("ln(-5)", x, res) == -3, true);
  EXPECT_EQ(model1->SmartCalc_m("log(-5)", x, res) == -3, true);
  EXPECT_EQ(model1->SmartCalc_m("10mod(5-5)", x, res) == -2, true);
  EXPECT_EQ(model1->SmartCalc_m("10mod(-x)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("10modx", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("xmod5", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("(-x)mod5", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("sqrt(x)", x, res) == -3, true);
  EXPECT_EQ(model1->SmartCalc_m("sqrt(-1)", x, res) == -3, true);
  EXPECT_EQ(model1->SmartCalc_m("sqrt(4)", x, res) == 0, true);
  EXPECT_EQ(model1->SmartCalc_m("(5^-3)", x, res) == -1, true);
}
...
TEST(Calc, MatematikaExp) {
  ...
  EXPECT_EQ(model1->SmartCalc_m("(1+2)e+2",  str_x, res),  -1); // потом обработать
  EXPECT_EQ(model1->SmartCalc_m("(-2)e+6",  str_x, res),  -1); // потом обработать
  EXPECT_EQ(model1->SmartCalc_m("sin(x)e+2",  str_x, res),  -1); // потом обработать
  EXPECT_EQ(model1->SmartCalc_m("(-2)e+sin(5)",  str_x, res),  -1); // потом обработать
  EXPECT_EQ(model1->SmartCalc_m("2e+(6+1)",  str_x, res),  -1); // потом обработать
  EXPECT_EQ(model1->SmartCalc_m("xe+x", str_x, res),  0);
  EXPECT_EQ(res == 10, true);
  EXPECT_EQ(model1->SmartCalc_m("65e+4", str_x, res),  0);
  EXPECT_EQ(res == 650000, true);
  EXPECT_EQ(model1->SmartCalc_m("0.5e+5", str_x, res),  0);
  EXPECT_EQ(res == 50000, true);
  EXPECT_EQ(model1->SmartCalc_m("-1.5e+5", str_x, res),  0);
  EXPECT_EQ(res == -150000, true);
  EXPECT_EQ(model1->SmartCalc_m("+0.5e+6", str_x, res),  0);
  EXPECT_EQ(res == 500000, true);
  EXPECT_EQ(model1->SmartCalc_m("0.5e-6", str_x, res),  0);
  EXPECT_EQ(res == 5e-07, true);
  EXPECT_EQ(model1->SmartCalc_m("0.5E+6", str_x, res),  0);
  EXPECT_EQ(res == 500000, true);
  EXPECT_EQ(model1->SmartCalc_m("0.5E-2", str_x, res),  0);
  EXPECT_EQ(res == 0.005, true);
  EXPECT_EQ(model1->SmartCalc_m("0.5 e+6", str_x, res),  0);
  EXPECT_EQ(res == 500000, true);
  EXPECT_EQ(model1->SmartCalc_m("0.5e+6+2", str_x, res),  0);
  EXPECT_EQ(res == 500002, true);
  EXPECT_EQ(model1->SmartCalc_m("0.5e+2*4", str_x, res),  0);
  EXPECT_EQ(res == 200, true);
  EXPECT_EQ(model1->SmartCalc_m("0.5e-3*4", str_x, res),  0);
